...
def build_dataset_from_txt(path_to_dataset, group_short_sentences=False, group_n_sentences=10):
    ...
    if not group_short_sentences:
        ...
        # shuffling sentences (they are clustered by similar syntactic structure)
        random.shuffle(train_data)
        random.shuffle(validation_data)
        random.shuffle(test_data)
        # concatenating N sentences in 1 to have less but longer input sentences
        g_N_s = group_n_sentences
        train_dataset = Dataset.from_dict({
            "doc_id": [train_data[i]["id"] for i in range(0, len(train_data), g_N_s)],
            "tokens": [" ".join([" ".join(train_data[i]["tokens"]) for i in range(i, min(i+g_N_s, len(train_data)))]).split() for i in range(0, len(train_data), g_N_s)],
            "labels": [" ".join([" ".join(train_data[i]["labels"]) for i in range(i, min(i+g_N_s, len(train_data)))]).split() for i in range(0, len(train_data), g_N_s)]
        })
        validation_dataset = Dataset.from_dict({
            "doc_id": [validation_data[i]["id"] for i in range(0, len(validation_data), g_N_s)],
            "tokens": [" ".join([" ".join(validation_data[i]["tokens"]) for i in range(i, min(i+g_N_s, len(validation_data)))]).split() for i in range(0, len(validation_data), g_N_s)],
            "labels": [" ".join([" ".join(validation_data[i]["labels"]) for i in range(i, min(i+g_N_s, len(validation_data)))]).split() for i in range(0, len(validation_data), g_N_s)]
        })
        test_dataset = Dataset.from_dict({
            "doc_id": [test_data[i]["id"] for i in range(0, len(test_data), g_N_s)],
            "tokens": [" ".join([" ".join(test_data[i]["tokens"]) for i in range(i, min(i+g_N_s, len(test_data)))]).split() for i in range(0, len(test_data), g_N_s)],
            "labels": [" ".join([" ".join(test_data[i]["labels"]) for i in range(i, min(i+g_N_s, len(test_data)))]).split() for i in range(0, len(test_data), g_N_s)]
        })
